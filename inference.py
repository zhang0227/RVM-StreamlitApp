...
def convert(opt):
    ...
    converter = Converter(opt.variant, opt.checkpoint, opt.device)
    converter.convert(
        input_source=opt.input_source,
        input_resize=opt.input_resize,
        downsample_ratio=opt.downsample_ratio,
        output_type=opt.output_type,
        output_composition=opt.output_composition,
        output_alpha=opt.output_alpha,
        output_foreground=opt.output_foreground,
        output_video_mbps=opt.output_video_mbps,
        seq_chunk=opt.seq_chunk,
        num_workers=opt.num_workers,
        progress=not opt.disable_progress
    )
